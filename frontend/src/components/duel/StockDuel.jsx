...
export default function StockDuel({ items = [] }) {
  ...
  if (!items || items.length < 2) {
    return (
      <div className="glass-panel rounded-2xl p-12 text-center text-slate-400">
        <Swords className="w-10 h-10 mx-auto text-teal-400 mb-3" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-2xl p-6 border border-white/10 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-violet-500/20 border border-violet-500/40 flex items-center justify-center text-violet-400 shadow-[0_0_20px_rgba(139,92,246,0.3)]">
              <Swords className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-slate-100 tracking-tight">
                  Stock Duel — Head-to-Head Factor Comparator
                </h2>
          ...
          {duelPresets.length > 0 && (
            <div className="flex items-center gap-2 overflow-x-auto">
              ...
              {duelPresets.map(preset => (
                <button
                  key={preset.label}
                  onClick={() => { setStockAKey(preset.a); setStockBKey(preset.b); }}
                  className="px-2.5 py-1 rounded-lg glass-panel hover:bg-slate-800 text-[11px] font-semibold text-slate-300 border border-white/10 shrink-0"
                >
                  {preset.label}
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-9 gap-4 items-center pt-2">
          {/* Fighter A Card */}
          <div className="md:col-span-4 glass-card rounded-2xl p-5 border-2 border-teal-500/30 bg-gradient-to-br from-teal-950/20 to-slate-900/80 space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              <select
                ...
                className="bg-slate-900 border border-white/10 rounded-xl px-3 py-1.5 text-base font-black text-slate-100 font-mono focus:border-teal-500 focus:outline-none"
              >
                {items.map(i => (
                  <option key={i.ticker} value={i.ticker}>
                    {i.ticker} • {i.companyName || i.ticker}
                  </option>
                ))}
              </select>
          ...
          <div className="md:col-span-1 flex flex-col items-center justify-center">
            ...
            <span className="text-[11px] font-mono font-black text-slate-500 uppercase tracking-widest mt-1">VS</span>
          </div>

          {/* Fighter B Card */}
          <div className="md:col-span-4 glass-card rounded-2xl p-5 border-2 border-violet-500/30 bg-gradient-to-br from-violet-950/20 to-slate-900/80 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-mono font-bold uppercase px-2 py-0.5 rounded bg-violet-500/20 text-violet-300">
                FIGHTER B (VIOLET)
              </span>
              <button
                onClick={() => openTradeModal(stockB.ticker)}
                className="text-[11px] font-mono text-violet-400 hover:underline flex items-center gap-1"
              >
                + Paper Trade <ArrowUpRight className="w-3 h-3" />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between gap-2">
              <select
                ...
                className="bg-slate-900 border border-white/10 rounded-xl px-3 py-1.5 text-base font-black text-slate-100 font-mono focus:border-violet-500 focus:outline-none"
              >
                {items.map(i => (
                  <option key={i.ticker} value={i.ticker}>
                    {i.ticker} • {i.companyName || i.ticker}
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-mono">
              <span className="font-bold text-teal-300">{stockA.ticker}: {aChange > 0 ? '+' : ''}{aChange}%</span>
              <span className="text-slate-400 font-sans font-semibold">24h Day Return</span>
              <span className="font-bold text-violet-300">{stockB.ticker}: {bChange > 0 ? '+' : ''}{bChange}%</span>
            </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-mono">
              <span className="font-bold text-teal-300">{stockA.ticker}: {aScore} pts</span>
              <span className="text-slate-400 font-sans font-semibold">Attention Triage Score</span>
              <span className="font-bold text-violet-300">{stockB.ticker}: {bScore} pts</span>
            </div>
            <div className="w-full h-3 rounded-full bg-slate-900 overflow-hidden flex">
              <div
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-mono">
              <span className="font-bold text-teal-300">{stockA.ticker}: {aZScore.toFixed(2)}σ</span>
              <span className="text-slate-400 font-sans font-semibold">Abnormal Volatility Z-Score</span>
              <span className="font-bold text-violet-300">{stockB.ticker}: {bZScore.toFixed(2)}σ</span>
            </div>
            <div className="w-full h-3 rounded-full bg-slate-900 overflow-hidden flex">
              <div
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-mono">
              <span className="font-bold text-teal-300">{stockA.ticker}: {aVol.toFixed(1)}x avg</span>
              <span className="text-slate-400 font-sans font-semibold">Volume Anomaly Multiple</span>
              <span className="font-bold text-violet-300">{stockB.ticker}: {bVol.toFixed(1)}x avg</span>
            </div>
            <div className="w-full h-3 rounded-full bg-slate-900 overflow-hidden flex">
              <div
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-mono">
              <span className="font-bold text-teal-300">{stockA.ticker}: {aRangePct}% of 52w range</span>
              <span className="text-slate-400 font-sans font-semibold">52-Week Range Position</span>
              <span className="font-bold text-violet-300">{stockB.ticker}: {bRangePct}% of 52w range</span>
            </div>
            <div className="w-full h-3 rounded-full bg-slate-900 overflow-hidden flex">
              <div
      ...
      <div className="glass-panel rounded-2xl p-6 border border-white/10 space-y-4 bg-gradient-to-r from-slate-900/90 via-slate-900/60 to-slate-900/90">
        <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
          <div className="p-3.5 rounded-xl bg-slate-900/70 border border-white/5 space-y-1">
            ...
            <div className="text-sm font-bold text-slate-100">{momentumWinner.ticker}</div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-900/70 border border-white/5 space-y-1">
            ...
            <div className="text-sm font-bold text-slate-100">{stabilityWinner.ticker}</div>
            <p className="text-[11px] text-slate-400 font-sans">
          ...
          <div className="p-3.5 rounded-xl bg-slate-900/70 border border-white/5 space-y-1">
            ...
            <div className="text-sm font-bold text-slate-100">r = {estimatedCorr > 0 ? '+' : ''}{estimatedCorr}</div>
            <p className="text-[11px] text-slate-400 font-sans">
              {estimatedCorr > 0.6
